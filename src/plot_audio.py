...
def plot_signal(sig, fs):
    """ Plot a signal in time."""
    seconds = np.arange(len(sig))/fs
    amp = np.max([np.max(np.abs(sig)), 1])
    plt.figure(figsize=(10,3))
    plt.plot(seconds, sig)
    plt.ylim([-amp,amp])
    plt.xlim([np.max(seconds), np.min(seconds)])
    plt.xlabel('Time [s]');
